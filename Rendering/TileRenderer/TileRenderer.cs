...
public static class TileRenderer
{
    public static BaseShape Tessellate(this MapFeatureData feature, ref BoundingBox boundingBox, ref PriorityQueue<BaseShape, int> shapes)
    {
        ...
        // modified with data from enum instead of string
        ...
        else if (feature.Properties.Any(p => p.Key == TerrainTypes.landuse && (p.Value.StartsWith("forest") || p.Value.StartsWith("orchard"))))
        {
            ...
            shapes.Enqueue(geoFeature, geoFeature.ZIndex);
                     ...
                     => p.Key == TerrainTypes.landuse && (p.Value.StartsWith("residential") || p.Value.StartsWith("cemetery") || p.Value.StartsWith("industrial") || p.Value.StartsWith("commercial") ||
                                                        ...
                                                        p.Value.StartsWith("brownfield"))))
        {
            ...
            shapes.Enqueue(geoFeature, geoFeature.ZIndex);
                     ...
                     => p.Key == TerrainTypes.landuse && (p.Value.StartsWith("farm") || p.Value.StartsWith("meadow") || p.Value.StartsWith("grass") || p.Value.StartsWith("greenfield") ||
